...
client = Cloudant.iam('ALapIE1wInqvwDermu5kKd-0SEOFKA_HWj8-UG45mzl2',connect=True)
...
#response = requests.get(url)
    render_template(
        'register.html', pred="Registration Successful, please login using your details")
else:
    render_template(
        'register.html', pred="You are already a member, please login using your details")
# login page
...
print(len(docs.all()))
if (len(docs.all()) == 0):
    render_template('login.html', pred="The username is not found.")
else:
...
render_template('prediction.html',
                prediction="Emergency !!! The Person is  drowining")
